...
{
  public:
    DateTime() : microSecondsSinceEpoch_(0){};

    /**
     * @brief Construct a new DateTime instance.
     *
     * @param microSec The microseconds from 1970-01-01 00:00:00.
     */
    explicit DateTime(int64_t microSec) : microSecondsSinceEpoch_(microSec){};
    ...
    /**
     * @brief Create a DateTime object that represents the current time.
     *
    ...
    static int64_t timezoneOffset()
    {
        static int64_t offset = -(
            DateTime::fromDbStringLocal("1970-01-03 00:00:00").secondsSinceEpoch() -
            2LL * 3600LL * 24LL);
        return offset;
    }

    /**
     * @brief Return a new DateTime instance that represents the time after some
     * seconds from *this.
     *
     * @param second
     * @return const DateTime
     */
    const DateTime after(double second) const;

    /**
     * @brief Return a new DateTime instance that equals to *this, but with zero
     * microseconds.
     *
     * @return const DateTime
     */
    const DateTime roundSecond() const;

    /// Create a DateTime object equal to * this, but numbers of hours, minutes,
     ...
     */
    static DateTime fromDbStringLocal(const std::string &datetime);
    /**
     * @brief From DB string to trantor UTC time.
     *
     * Inverse of toDbString()
     */
    static DateTime fromDbString(const std::string &datetime);
     ...
     * @return false
     */
    bool isSameSecond(const DateTime &date) const
    {
        return microSecondsSinceEpoch_ / MICRO_SECONDS_PRE_SEC ==
