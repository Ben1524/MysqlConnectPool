...
const DateTime DateTime::date()
{
    ...
    return DateTime(seconds * MICRO_SECONDS_PRE_SEC + tv.tv_usec);
#else
    timeval tv;
    gettimeofday(&tv, NULL);
    int64_t seconds = tv.tv_sec;
...
const DateTime DateTime::after(double second) const
{
    return DateTime(static_cast<int64_t>(microSecondsSinceEpoch_ +
                                     second * MICRO_SECONDS_PRE_SEC));
}
const DateTime DateTime::roundSecond() const
{
    return DateTime(microSecondsSinceEpoch_ -
                (microSecondsSinceEpoch_ % MICRO_SECONDS_PRE_SEC));
}
const DateTime DateTime::roundDay() const
{
    struct tm t={0};
    time_t seconds =
    ...
    t.tm_min = 0;
    t.tm_sec = 0;
    return DateTime(mktime(&t) * MICRO_SECONDS_PRE_SEC);
}
struct tm DateTime::tmStruct() const
{
    time_t seconds =
        static_cast<time_t>(microSecondsSinceEpoch_ / MICRO_SECONDS_PRE_SEC);
    struct tm tm_time;
...
std::string DateTime::toCustomFormattedString(const std::string &fmtStr,
...
void DateTime::toCustomFormattedString(const std::string &fmtStr,
                                   char *str,
                                   size_t len) const
{
    // not safe
...
std::string DateTime::toFormattedStringLocal(bool showMicroseconds) const
{
    //  std::cout<<"toFormattedString"<<std::endl;
    char buf[128] = {0};
    time_t seconds =
...
std::string DateTime::toDbStringLocal() const
{
    char buf[128] = {0};
    time_t seconds =
        static_cast<time_t>(microSecondsSinceEpoch_ / MICRO_SECONDS_PRE_SEC);
...
DateTime DateTime::fromDbStringLocal(const std::string &datetime)
{
    unsigned int year = {0}, month = {0}, day = {0}, hour = {0}, minute = {0},
                 second = {0}, microSecond = {0};
    std::vector<std::string> &&v = utils::splitString(datetime, " ");
    if (2 == v.size())
    {
        // date
        std::vector<std::string> date = utils::splitString(v[0], "-");
        if (3 == date.size())
        {
            year = std::stol(date[0]);
            month = std::stol(date[1]);
            day = std::stol(date[2]);
            std::vector<std::string> time = utils::splitString(v[1], ":");
            if (2 < time.size())
            {
                hour = std::stol(time[0]);
                minute = std::stol(time[1]);
                auto seconds = utils::splitString(time[2], ".");
    ...
    return cxk::DateTime(year, month, day, hour, minute, second, microSecond);
}
DateTime DateTime::fromDbString(const std::string &datetime)
{
    return fromDbStringLocal(datetime).after(
        static_cast<double>(timezoneOffset()));
}

std::string DateTime::toCustomFormattedStringLocal(const std::string &fmtStr,
                                               bool showMicroseconds) const
{
    char buf[256] = {0};
    time_t seconds =
